...
import { postRegistrarUsuario } from '../../utils/api'
...
const handleSubmit = async () => {
  ...
  const fieldsToValidate = ['email', 'password', 'user']

  postRegistrarUsuario(formState, resetForm, fieldsToValidate, dataBody, () => {
    router.push('/login')
  })
}
</script>
...
<template>
  <DefaultAuthCard
    subtitle="Estás a unos pasos de entrar a un mundo de historias y conocimiento"
    title="Registrate en Booksio"
    class="min-h-screen flex flex-col justify-center items-center"
  >
    <form @submit.prevent="handleSubmit">
      <InputGroup label="Nombre de usuario" name="user" id="user" type="text" placeholder="usuario">
        <svg
          class="fill-current"
          width="22"
          height="22"
